...
import { 
  ...
  Wallet, CreditCard, QrCode, ArrowUpRight, RefreshCw, Settings, Trash2
} from 'lucide-react';
...
export default function DashboardUMKM() {
  ...
  const handlePostingSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast("Gagal memposting pekerjaan.", "error");
      }
    } catch (err) {
      console.error(err);
  ...
  if (selectedApplicantsId) {
    ...
    if (proj && proj.applications) {
      currentApplicants = proj.applications.map((app, index) => ({
        id: app.mahasiswa?.id || `dummy-${index}`,
        nama: app.mahasiswa?.name || "Mahasiswa",
        univ: "Universitas Indonesia", // Fallback dummy
        jurusan: "S1 Sistem Informasi - Sem 6", // Fallback dummy
        rating: 4.8, 
        jobsDone: Math.floor(Math.random() * 10) + 1, 
        avatar: "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=120&q=80",
        skills: ["Digital Marketing", "Content Creation", "Ms. Office"],
        pesanLamaran: "Halo Kak! Saya siap bekerja keras dan memiliki pengalaman yang relevan dengan proyek ini.",
        portfolioImages: [
                  ...
                  return (
                    <div className="space-y-6">
                      ...
                      <div className="pt-4 border-t border-slate-200/80 flex flex-col sm:flex-row gap-3">
                        <button 
                          onClick={() => {
                            if (typeof candidate.id === 'string' && candidate.id.startsWith('dummy')) {
                              showToast("Pelamar ini adalah data simulasi (dummy), tidak bisa mengirim pesan.", "error");
                              return;
                            }
